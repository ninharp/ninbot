...
sub setup_IRC {
  ...
  $self->read_Config;
  my $IRC = $self->{_IRC};
  $self->{conn} = $IRC->newconn(Nick    => $self->{config}->{irc_nickname},
...
sub setup_Handler {
  my $self = shift;
  my $conn = $self->{conn};
  if ($conn->add_global_handler('376', \&IRC_on_connect) and
      $conn->add_global_handler('disconnect', \&IRC_on_disconnect) and
      $conn->add_global_handler('433', \&IRC_on_nick_in_use) and
      $conn->add_global_handler('join', \&IRC_on_join) and
      $conn->add_global_handler('part', \&IRC_on_part) and
      $conn->add_global_handler('quit', \&IRC_on_quit) and
      $conn->add_global_handler('nick', \&IRC_on_nick_change) and
      $conn->add_global_handler('public', \&IRC_on_public) and
      $conn->add_global_handler('kick', \&IRC_on_kick) and
      $conn->add_global_handler('msg', \&IRC_on_private) and
      $conn->add_global_handler('invite', \&IRC_on_invite) and
      $conn->add_global_handler('cversion', \&IRC_on_ctcp_version) and
      $conn->add_global_handler('namreply', \&IRC_on_names_reply)) {
    return 1;
  }
  else {
    return 0;
...
sub handle_Event {
  ...
  my @n_c_calc = $calc->get_Calc("data-event-$from_channel-$from_nick-$event_type");
...
sub IRC_on_join {
  ...
  $self->handle_Event("onjoin", $event) if $irc->{_nick} !~ m/$event->{nick}/i;
#  print "IRC:\n".Dumper($irc)."\n\n";
#  print "EVENT:\n".Dumper($event)."\n\n";
...
#  if ($message =~ m/^(calc|match|${trigger}index|${trigger}list)/i) {
  if ($message =~ m/^(data|match|${trigger}index|${trigger}list)/i) {
	...
	} else { $self->log(2, "<Main:IRC:pub> Requested to ban ".$from_channel." but it is not in Channel list"); }
  }
	...
	if ($self->{_CHANNEL}->{$from_channel}->isMuted() == 0) {
		...
		if (defined $calc[1]) {
			...
			my ($calc_name, $calc_text, $calc_nick, $calc_date, $calc_changed, $calc_mode, $calc_flag, $calc_level) = @calc;
...
sub IRC_on_private {
  ...
  if ($from_channel =~ m/^(?:\!|\+)/) {
	...
	$irc->privmsg($from_nick, "Du darfst das nicht mit Level $level!");
	$self->log(3, "<Main:IRC:priv> Denied PrivCmd Join $1 from $from_nick");
      }
    }
	...
	$self->log(3, "<Main:IRC:priv> User $from_nick already identified!");
	...
	my $sth = $self->{_DBH}->update("user", {"handle", $from_nick, "password", $password}); #prepare("UPDATE user SET password='$password' WHERE handle = '$from_nick'");
	$sth->execute;
	...
	$self->{conn}->quit("Kill requested by $from_nick ($kill_msg)") if defined $kill_msg;
	$self->{conn}->quit("Kill requested by $from_nick") if defined !$kill_msg;
	$self->save_Config;
	exit(1);
      }
    }
