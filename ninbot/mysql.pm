...
sub new {
  ...
  $self->{_fields_user} = ();
  $self->{_fields_calc} = ();
  $self->{_DBH} = DBI->connect($self->{_BOT}->{config}->{sql_dsn}, $self->{_BOT}->{config}->{sql_user}, $self->{_BOT}->{config}->{sql_password}) or print "Can't connect to the DB: $DBI::errstr\n" or die;
  $self->_get_fields;
...
sub _get_fields {
  ...
  $sth->finish;
  @{$backend->{_fields_user}} = @fields;  undef @fields;
  ...
  $sth->finish;
  @{$backend->{_fields_calc}} = @fields;  undef @fields;
}
...
sub add {
  ...
  $self->log(4, "<mySQL> [$database] add($database, '".@values."[".scalar(@values)."]')");
  my $ret = 0;
  my $counter = 0;
...
sub match {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
    ...
    if (scalar(@ret) <= 0) {
      ...
      $self->log(4, "<mySQL> [$database] match = Found $num_entries Entries ('$like')[$column]");
    }
    $sth->finish();
  }
  else {
...
sub update {
  ...
  my $sth = $backend->{_DBH}->prepare("UPDATE $database SET $up_fields WHERE $index_field='$data[0]'");
  $ret = $sth->execute;
...
sub select {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
    ...
    if ($sth->execute) {
      ...
      $self->log(4, "<mySQL> [$database] select = Found entry!") if (defined $ret[0] and $ret[$column] =~ m/$like/i);
    }
    $sth->finish();
    if (scalar(@ret) <= 0) {
      $self->log(4, "<mySQL> [$database] select = Nothing found! ('$like')[$column]");
    }
  }
  else {
    $self->log(4, "<mySQL> [$database] select = Wrong database specified!");
  }
  return @ret;
}
...
sub select_all {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
    ...
    $self->log(4, "<mySQL> [$database] select_all = Wrong database specified!");
  }
  return @ret;
}
