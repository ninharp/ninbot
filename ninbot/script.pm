...
# CalcScript Module for ninBOT - http://ninbot.sourceforge.net
# $Header: /cvsroot/ninbot/ninbot/ninbot/script.pm,v 1.14 2003/05/08 10:39:36 prahnin Exp $
#
...
sub new {
  ...
  $self->{_BOT}->log(3, "<Script> Script Object loaded! (".$self->{'-command'}.")");
  return $self;
}
...
sub DESTROY {
  my $self = shift;
  $self->{_BOT}->log(3, "<Script> Script Object Destroyed. (".$self->{'-command'}.")");
  delete $self->{-param};
  delete $self->{-command};
  delete $self->{-return};
}
  ...
  for (my $cmd_count = 0; $cmd_count < scalar(@cmds); $cmd_count++) {
	...
	if ($com =~ m/sleep\s*(\d+)/i) {
	  ...
	  for (my $i = $cmd_count+1; $i <= scalar(@cmds); $i++) {
	    $script_temp .= $cmds[$i].";"; # if defined $cmds[$i+1];
	  }
	  my $after_script = sub {
	...
	if (defined $stack[0]) {
	  ...
	  $conn->privmsg($chan, "Stack ".$stack_name.": ".$output);
	}
	else {
	...
	my $send_text = "* ".$r_name." = ".$r_text." [".$r_author.", ".$r_date.", ".$r_calc[6]."/".$r_calc[7]."]";
	...
	#print "OLD=$cmd_count\n";
	$self->$command($do_this, $nick, $chan, $level, $param, \$cmd_count, @cmds);
	#print "NEW=$cmd_count\n";
...
sub nif {
  ...
  if ($do_this =~ m/^\s*(.*?)\s*((?:=|eq|\!=|ne))\s*(.*?)$/i) {
    ...
    $count = scalar(@cmds) if $ifend == 0;#$self->{_COUNT} = scalar(@cmds) if $ifend == 0;
    ${$cmd_count} = $count;
  }
  else {
...
#  }
  ...
  if ($do_this =~ m/^\s*(.*?)\s*((?:=|eq|\!=|ne))\s*(.*?)\s+then\s+\{?(.*?)\}?\s*(else\s+\{?(.*)\}?)?$/i) {
    ...
    $if_script = join(";", @if_script_cmds);undef @if_script_cmds;
    $if_else = join(";", @if_else_cmds);undef @if_else_cmds;
    $if_script =~ s/^\s*(.*)\s*$/$1/;
