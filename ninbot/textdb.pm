...
# Calc Module [textDB Backend] for ninBOT - http://ninbot.sourceforge.net
# $Header: /cvsroot/ninbot/ninbot/ninbot/textdb.pm,v 1.2 2003/04/28 17:04:34 prahnin Exp $
#
...
sub add {
  ...
  $self->log(4, "<textDB> [$database] add($database, '".@values."[".scalar(@values)."]')");
  my $ret = 0;
  my $counter = 0;
  if ($database =~ m/^(?:user|calc)$/i) {
	...
	close(DB);
	$self->log(4, "<textDB> [$database] add = Added successfully! ($values[0])");
	$ret = 1;
      }
      else {
	$self->log(4, "<textDB> [$database] add = Cannot open file $file for writing!");
      }
    }
    else {
      $self->log(4, "<textDB> [$database] add = Cannot open file $file.");
      exit(1);
    }
  }
  else {
...
sub match {
 ...
 my $db_file = lc($database)."_file";
	...
	if ($row[$column] =~ m/$like/i) {
      ...
      $self->log(4, "<textDB> [$database] match = Cannot open file $file.");
      exit(1);
      }
    if (scalar(@ret) <= 0) {
      ...
      $self->log(4, "<textDB> [$database] match = Found ".scalar(@ret)." entries!");
    }
  }
  else {
    $self->log(4, "<textDB> [$database] match = Wrong database specified!");
  }
  return @ret;
}
...
sub del {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
	...
	$self->log(4, "<textDB> [$database] del = Deleted successfully!") if ($ret != 0);
	$ret = 1;
	}
      }
	...
	if(open(DB, ">$file")) {
	  ...
	  $self->log(4, "<textDB> [$database] del = Cannot open file $file for writing!");
	}
      }
    }
    else {
      $self->log(4, "<textDB> [$database] del = Cannot open file $file.");
      exit(1);
    }
  }
  else {
...
sub update {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
	...
	if ($row[$column] !~ m/^$data[$column]$/i) {
	  ...
	  $self->log(4, "<textDB> [$database] update = Updated successfully!") if ($ret != 0);
	  $ret = 1;
	}
      }
      if ($ret == 1) {
	if(open(DB, ">$file")) {
	  ...
	  $self->log(4, "<textDB> [$database] update = Cannot open file $file for writing!");
	}
      }
    }
    else {
      $self->log(4, "<textDB> [$database] update = Cannot open file $file.");
      exit(1);
    }
  }
  else {
    $self->log(4, "<textDB> [$database] update = Wrong database specified!");
  }
  return $ret;
}
...
sub select {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
	...
	if ($row[$column] =~ m/$like/i) {
      ...
      $self->log(4, "<textDB> [$database] select = Cannot open file $file.");
      exit(1);
      }
    if (scalar(@ret) <= 0) {
    ...
    $self->log(4, "<textDB> [$database] select = Wrong database specified!");
  }
  return @ret;
}
...
sub select_all {
  ...
  if ($database =~ m/^(?:user|calc)$/i) {
	...
	push(@ret, $return);
      ...
      $self->log(4, "<textDB> [$database] select_all = Cannot open file $file.");
      exit(1);
      }
    if (scalar(@ret) <= 0) {
      $self->log(4, "<textDB> [$database] select_all = Nothing returned!");
    }
    else {
      $self->log(4, "<textDB> [$database] select_all = Returned ".scalar(@ret)." entries!");
    }
  }
  else {
    $self->log(4, "<textDB> [$database] select_all = Wrong database specified!");
  }
  return @ret;
}
