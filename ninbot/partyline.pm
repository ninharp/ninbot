...
# Module for IRC bot (c) by ninharp
# partyline.pm - Partyline Module
...
sub handle {
  ...
  foreach my $request (@{$partyline->{ready}->{$client}}) {
    ...
    if (!defined $partyline->{authorized}->{$client}) {
      ...
      $partyline->{connected}->{$partyline->{authorized}->{$client}->{login}} = $client;
      $partyline->{outbuffer}->{$client} = $partyline->{authorized}->{$client}->{login}." you have succesfully identified to the partyline.\r\n";
    }
    elsif ($partyline->{authorized}->{$client}->{status} >= 2) {
      print "(".$partyline->{authorized}->{$client}->{login}.") ".$request."\n" if $request !~ m/^\W*$/i;
      my $clients = $partyline->{authorized};
      my %clients = %{$clients};
      my $echo = 1;
      foreach my $c (keys %clients) {
	if ($c ne $client or $echo == 1) {
	  $partyline->{outbuffer}->{$c} = "<".$partyline->{authorized}->{$client}->{login}."> ".$request."\r\n" if $request !~ m/^\W*$/i;
	}
      }
    }
    else {
...
sub nonblock {
  ...
  fcntl($socket, F_SETFL, $flags | O_NONBLOCK) if defined $socket and $flags;
}
...
#		delete $partyline->{log}->{$client};
	...
	delete $partyline->{outbuffer}->{$client} unless length $partyline->{outbuffer}->{$client};
      } else {
