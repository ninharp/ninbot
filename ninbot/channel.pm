...
sub isMuted {
    ...
    my $self         = &main::get_Self;
    $self->log( 4,
        "<Channel> isMuted $channel_self->{_NAME} = $channel_self->{_MUTE}" );
...
sub add_name {
    ...
    if ( defined $channel_self->{_NAMES} ) {
        if ( $channel_self->{_NAMES} !~ m/$name/ ) {
            $self->log( 3,
                    "<Channel> Adding Nickname $name to Channel "
                  . $channel_self->{_NAME}
                  . " Names list" );
...
sub del_name {
    ...
    if ( $channel_self->{_NAMES} =~ m/$name/ ) {
        $self->log( 3,
                "<Channel> Deleting Nickname $name from Channel "
              . $channel_self->{_NAME}
              . " Names list" );
...
sub change_name {
    ...
    if ( $channel_self->{_NAMES} =~ m/$name/ ) {
        $self->log( 3,
                "<Channel> Changing Nickname $name to $newname on Channel "
              . $channel_self->{_NAME}
              . " Names list" );
...
sub fill_names {
    ...
    $self->log( 4,
            "<Channel> Fill in Channel name list for "
          . $channel_self->{_NAME} . ": "
          . $out );
