...
sub new {
  ...
  foreach my $user_string (@users) {
    ...
    my $new_user_instance = ninbot::user->new( '-nickhandle' => $user_handle, #string
					       '-flags' => $user_flag,
					       '-hosts' => [ @hosts ],
...
sub active_User {
  ...
  foreach my $tmp_user (@$users) {
    if($tmp_user->chk_hostmask($host)) {
      if(  $tmp_user->{'-passwd'} eq crypt( $password, $tmp_user->{'-passwd'} )){
...
sub is_Active {
  ...
  foreach my $tmp_user (@$users) {
    if($tmp_user->{'-current_mask'} and $tmp_user->{'-current_mask'} eq $from and $tmp_user->is_identifyed){
      # user erkannt
      return 1;
    }
  }
