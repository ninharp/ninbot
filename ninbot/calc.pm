package ninbot::calc;
# Main Calc Module for ninBOT - http://ninbot.sourceforge.net
# $Header: /cvsroot/ninbot/ninbot/ninbot/calc.pm,v 1.4 2003/05/08 10:38:12 prahnin Exp $
#
...
sub add_Calc {
  ...
  if (defined $calc[1]) {
    ...
    my $changed = "";
    my $level = 0;
    my @add = ( $name, $text, $author, $date, $changed, $mode, $flag, $level );
...
sub plus_Calc  {
  ...
  my $changed = "";
  my $level = 0;
  my @add = ( $calc_name, $text, $author, $date, $changed, $mode, $flag, $level );
  $ret = $dbh->add("calc", @add);
  if ($ret == 2 or $ret == -1) {
      $ret = -1;
    }
...
sub Handler {
  ...
  # Set Calc
    ...
    my $calc_name = $1;
    my $calc_text = $2;
    my $calc = $calc_self->add_Calc($calc_name, $from_nick, $calc_text);
    if ($calc <= 0) {
  ...
  # Calc
    ...
    if ($calc_name !~ m/^\W*$/i) {
		...
		$conn->privmsg($from_channel, "* $calc[0] = $new_calc [$calc[2], $calc[3], $calc[6]/$calc[7]]");
		}
		else {
		$conn->privmsg($from_channel, "* $calc[0] = $calc[1] [$calc[2], $calc[3], $calc[6]/$calc[7]]");
		}
		}
		else {
  ...
  # Remove Calc
    ...
    my $calc_name = $1;
    ($calc_name, undef) = split(/\s*=\s*/, $calc_name) if $calc_name =~ m/=/;
  ...
  # Calc Flag
    ...
    if (!defined $acc_calc[1]) {
      ...
      if ($author eq $from_nick or $acc_calc[6] =~ m/w/i and $level >= $acc_calc[7]) {
  ...
  # Calc Level
    ...
    for (my $i = $start_com; $i<$end_com; $i++) {
      $commands .= $com_list[$i];
      $commands .= ", " if ($i+1) < $end_com and defined $com_list[$i+1];
    }
